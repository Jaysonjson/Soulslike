package json.jayson.common.objects.blockitems;

import json.jayson.common.registries.SoulsBlocks;
import net.minecraft.network.chat.Component;
import net.minecraft.world.item.BlockItem;
import net.minecraft.world.item.ItemStack;
import net.minecraft.world.item.TooltipFlag;
import net.minecraft.world.level.Level;
import net.minecraft.world.level.block.Block;
import org.jetbrains.annotations.Nullable;

import java.util.List;

public class HoverTextBlock extends BlockItem {

    String text = "";
    public HoverTextBlock(Block p_40565_, Properties p_40566_, String text) {
        super(p_40565_, p_40566_);
        this.text = text;
    }

    @Override
    public void appendHoverText(ItemStack p_40572_, @Nullable Level p_40573_, List<Component> p_40574_, TooltipFlag p_40575_) {
        p_40574_.add(Component.translatable(text));
        super.appendHoverText(p_40572_, p_40573_, p_40574_, p_40575_);
    }
}
